...
class PyGBAEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._current_scale = scale_factor  # Track current scale
    ...
    def step(self, action_id):
        ...
        truncated = False
        self._step += 1 # moved step increment to trigger truncated
    ...
    def _update_window_size(self):
        if self._screen is not None and self._current_scale != self.scale_factor:
            base_dims = self.gba.core.desired_video_dimensions()
            scaled_dims = (int(base_dims[0] * self.scale_factor), int(base_dims[1] * self.scale_factor))
            pygame.display.quit()
            pygame.display.init()
            self._screen = pygame.display.set_mode(scaled_dims, pygame.DOUBLEBUF | pygame.HWSURFACE)
            pygame.display.set_caption("PyGBA")
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            ...
            if surf.get_size() != self._screen.get_size():
                surf = pygame.transform.scale(surf, self._screen.get_size())
